...
// FILE NAME    : serdes_8b10b_cpri_tx_scrambler.v
// AUTHOR       : Wang Kepan
// Department   : ZTE-BBU
...
//x                                    |----xxx1
//x                                    |----xxx2
//x serdes_8b10b_cpri_tx_scrambler-----|----xxx3
//x                                    |----xxx4
//x                                    |----xxx5
//-----------------------------------------------------------------
// Release History:
...
module  serdes_8b10b_cpri_tx_scrambler
        #(
       ...
       ///Input signals                                                                                 
         ...
         input  wire [30:0]                  I_scramble_cpri_seed ,///cpri seed                         
...
reg    [C_DATA_WIDTH   - 1:0]  S_cpri_data_b_s_1d    ; /// I_tx_data_b_s delay 1 clock 
...
always @ ( posedge I_opt_tx_clk )
    begin
       S_scramble_enable <= I_scramble_en && ((I_opt_rate_sel == 4'b0011) ||
...
always @ ( posedge I_opt_tx_clk )
    begin
       case ({I_opt_rate_sel,S_data_width})
          6'b0011_00: S_chip_clk_cnt_sel <= 7'd63 ;///16bit
          6'b0011_01: S_chip_clk_cnt_sel <= 7'd31 ;///32bit 
          6'b0011_10: S_chip_clk_cnt_sel <= 7'd15 ;///64bit
...
assign  S_mask_word_en = (S_chip_clk_cnt == 7'd0) && ((S_cw_cnt[5:0] == 6'd0 ) || (S_cw_cnt[5:0] == 6'd2));
...
assign S_current_value = {I_scramble_cpri_seed[3] ^ I_scramble_cpri_seed[0],I_scramble_cpri_seed};
...
always @ ( posedge I_opt_tx_clk or posedge I_opt_tx_rst )
    if(I_opt_tx_rst)
        begin
           S_cpri_data_b_s_1d <= {C_DATA_WIDTH{1'b0}} ;
        end    
    else
        begin
           S_cpri_data_b_s_1d <= I_tx_data_b_s ;
        end
...
always @ ( posedge I_opt_tx_clk or posedge I_opt_tx_rst )
    if(I_opt_tx_rst)
        ...
        begin
           case (S_data_width)
              2'b00 : O_tx_data_a_s[C_DATA_WIDTH-1:0] <=  (!S_chip_clk_cnt[0]) ? S_cpri_data_b_s_1d[15:0] ^ (S_current_value_16bit[15: 0] & S_mask_word)
                                                                               : S_cpri_data_b_s_1d[15:0] ^ (S_current_value_16bit[31:16] & S_mask_word);

              2'b01 : O_tx_data_a_s[C_DATA_WIDTH-1:0] <= {S_cpri_data_b_s_1d[31:16] ^  S_current_value_32bit[31:16],
                                                          S_cpri_data_b_s_1d[15: 0] ^ (S_current_value_32bit[15: 0] & S_mask_word)};

              2'b10 : O_tx_data_a_s[C_DATA_WIDTH-1:0] <= {S_cpri_data_b_s_1d[63:16] ^  S_current_value_64bit[63:16],
                                                          S_cpri_data_b_s_1d[15: 0] ^ (S_current_value_64bit[15: 0] & S_mask_word)};
              default : ;
           endcase
        end
    else
        begin
           O_tx_data_a_s[C_DATA_WIDTH-1:0] <= S_cpri_data_b_s_1d[C_DATA_WIDTH-1:0];
        end
